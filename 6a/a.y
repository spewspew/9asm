...
%token	<lval>	LCONST LFP LPC LSB LHERE LBACK LFWD
...
|	con '(' LBACK ')'
	{
		...
		if(pass == 2){
			...
			$$.type = D_BRANCH;
			$$.offset = -1;
			for(curlab = locl[$1]; curlab; curlab = curlab->next){
				if(pc > curlab->p){
					$$.offset = curlab->p;
					break;
				}
			...
			if($$.offset == -1)
				yyerror("no back label defined for %d", $1);
		}
	}
|	con '(' LFWD ')'
	{
		...
		if(pass == 2){
			...
			if($$.offset == -1)
				yyerror("no forward label defined for %d", $1);
		}
	}
|	LNAME offset
	{
